...
public class OuterRecycleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public OuterRecycleAdapter(List<Object> list, boolean selectType,
                               Calendar startCalendarDate, Calendar endCalendarDate,
                               DayTimeEntity startDayTime, DayTimeEntity endDayTime,boolean pasttime) {
        try {
            listMap = new HashMap<>();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 1) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_recyclerver_item_outer, parent, false);
            return new OuterRecycleViewHolder(v);
        } else {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_recyclerver_item_inner, parent, false);
            InnerViewHolder holder = new InnerViewHolder(view, startCalendarDate, endCalendarDate, startDayTime, endDayTime,pasttime);
            DateOnclickListener listener = new DateOnclickListener(selectType,pasttime, startDayTime, endDayTime,this);
